...
__all__ = ["LSSViewTransformer","LSSViewTransformerBEVDepth", "LSSViewTransformerBEVStereo"]
...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, cam2coor, cam2imgs, post_rots, post_trans,
        ...
        #     img2lidar = projection_mat.inverse()
        #     points = self.frustum.to(cam2lidar).unsqueeze(0).unsqueeze(0).repeat(B,N,1,1,1,1)
        #     points = torch.cat((points[..., :2] * points[..., 2:3], points[..., 2:3]),dim=-1)
        #     points_extend = torch.cat([points, torch.ones_like(points[..., :1])], dim=-1)
        #     points = torch.matmul(img2lidar[:,:,None,None,None],points_extend[:,...,None]).squeeze(-1)[..., :3]
    ...
    def view_transform_core(self, input, depth, tran_feat,projection_mat=None):
        ...
        if self.accelerate:
            ...
            coor = self.get_lidar_coor(*input[1:7],projection_mat=projection_mat)
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def get_mlp_input(self,view_tran_components):
        cam2lidar, intrin, post_rot, post_tran, bda = view_tran_components
        ...
        mlp_input = torch.stack([
            ...
            bda[:, :, 1, 1],
            bda[:, :, 2, 2],], dim=-1)
        cam2lidar = cam2lidar[:,:,:3,:].reshape(B, N, -1)
        mlp_input = torch.cat([mlp_input, cam2lidar], dim=-1)
        return mlp_input
    ...
    def forward(self, input,projection_mat,mlp_input, stereo_metas=None):
        (x, sensor2ego, intrins, post_rots, post_trans, bda) = input
        B, N, C, H, W = x.shape
        x = x.view(B * N, C, H, W)
