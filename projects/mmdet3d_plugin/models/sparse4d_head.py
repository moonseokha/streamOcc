...
#         else:
#             k=1
#         self.matcher_o2m = Stage2Assigner(allow_low_quality_matches=allow_low_quality_matches,k=k,class_score_thr=0.3)
...
#                 voxel_feature,_ ,_= self.layers[i](
...
#                     #     or i == len(self.operation_order) - 1
#                     #     or self.prediction_length == self.num_img_decoder -1
#                     # ),
#                 )
            ...
            if op == "temp_gnn":
                ...
                if self.training:
                    ...
                    if self.use_nms_filter:
                        ...
                        # o2m_cls = classification[-1].clone().detach()[:,:num_free_instance]
                        # # data_reg_target = self.sampler.encode_reg_target(metas[self.gt_reg_key],o2m_reg.device)
                        # box_origin = metas[self.gt_reg_key][0].clone()
                        # _, _, _, _, _ ,_,o2o_indices= self.sampler.sample(
                        # o2m_cls.clone(),
                        # o2m_reg.clone(),
                        # metas[self.gt_cls_key],
                ...
                if "voxel_concat" in self.operation_order:
                    ...
                    if self.operation_order[-1] == "interaction_gnn":
                        if i != len(self.operation_order) - 2:
                            anchor_embed = self.anchor_encoder(anchor)
                    else:
                        if i != len(self.operation_order) - 1:
                        ...
                        #     o2m_reg = prediction[-1].clone().detach()[:,:900][..., : len(self.reg_weights)]
                        #     o2m_cls = classification[-1].clone().detach()[:,:900]
                        #     # data_reg_target = self.sampler.encode_reg_target(metas[self.gt_reg_key],o2m_reg.device)
                        #     box_origin = metas[self.gt_reg_key][0].clone()
                        #     _, _, _, _, _ ,_,o2o_indices= self.sampler.sample(
                        #     o2m_cls.clone(),
