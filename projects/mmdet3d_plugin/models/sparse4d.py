...
#         if self.head is not None:
#             if self.use_DFA3D:
#                 model_outs,voxel_feature,up_vox_occ = self.head(feature_maps=feat_flatten, voxel_feature=voxel_feature, metas=data, occ_list=occ_list, depth = dpt_dist_flatten, spatial_shapes=spatial_shapes, level_start_index=level_start_index)
#             else:
#                 model_outs,voxel_feature,up_vox_occ = self.head(feature_maps=feature_maps, voxel_feature=voxel_feature, metas=data, occ_list=occ_list, depth = depths_voxel,occ_pos=occ_pos,ref_3d=ref_3d)
...
#             loss_occ,mask_head_occ = self.forward_mask_decoder_train(voxel_feature, voxel_feature_list, origin_feature_maps, data,instance_feature ,origin_voxel_feat=origin_voxel_feat,matching_indices=matching_indices,up_vox_occ=up_vox_occ)
#             output.update(loss_occ)
#             if self.no_maskhead_infertime is not True:
...
#         # pdb.set_trace()
#         outs,compact_occ = self.mask_decoder_head(voxel_feats,voxel_feature_list=voxel_feature_list,mlvl_feats=mlvl_feats,threshold=self.occupy_threshold,instance_queries=instance_queries_list,origin_voxel_feat=origin_voxel_feat,full_occ=up_vox_occ, **data)
