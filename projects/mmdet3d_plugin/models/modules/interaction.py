...
#     # corners_norm = corners_norm - torch.tensor([0.5, 0.5, 0.5])
#     # # Calculate corners
#     # corners = boxes[..., None, [W, L, H]] * corners_norm.reshape(1, 8, 3).to(boxes.device)
    
#     # # Rotate around z axis
#     # rot_cos = boxes[...,COS]
#     # rot_sin = boxes[...,SIN]
#     # rot_mat = torch.eye(3).unsqueeze(0).unsqueeze(0).repeat(boxes.shape[0],boxes.shape[1], 1, 1)
#     # rot_mat[..., 0, 0] = rot_cos
#     # rot_mat[..., 0, 1] = -rot_sin
#     # rot_mat[..., 1, 0] = rot_sin
#     # rot_mat[..., 1, 1] = rot_cos
#     # corners = torch.matmul(rot_mat.unsqueeze(2).to(corners.device), corners.unsqueeze(-1)).squeeze(-1)
...
#         #     vox_feat = query.view(B, H, W, D, embed)
...
#             # # 벡터화된 방식으로 모든 가능한 voxel 위치 계산
#             # grid_ranges = [torch.arange(min_pos[i, dim].item(), max_pos[i, dim].item() + 1, device=vox_feat.device)
#             #             for dim in range(3) for i in range(vox_pos.shape[0])]
#             # mesh_index = torch.cartesian_prod(*grid_ranges).view(vox_pos.shape[0], -1, 3)

#             # if self.img_to_vox:
#             #     i2v_pos.append(mesh_index.view(-1, 3))

#             # # Flatten the mesh indices
#             # mesh_index_flat = (mesh_index[..., 1] * self.vox_size[1] * self.vox_size[2] +
#             #                 mesh_index[..., 0] * self.vox_size[2] +
#             #                 mesh_index[..., 2]).view(-1)

#             # mesh_index_flat = mesh_index_flat.clamp(0, self.vox_size[0] * self.vox_size[1] * self.vox_size[2] - 1) + b * vox_flatten_size
#             # unique_indexes = torch.unique(mesh_index_flat)
...
#                 vox_with_query = self.cross_attn_Q2V(x=(selected_agent_feat,vox_feat.reshape(-1,self.embed_dims)),edge_index=edge_index_Q2V,edge_attr=selected_agent_pos)
#             else:
#                 vox_with_query = self.cross_attn_Q2V(x=(selected_agent_feat,vox_feat.reshape(-1,self.embed_dims)),edge_index=edge_index_Q2V)
...
#             # mask = agent_feat.sum(1).sum(1).bool()
#             # for i,batch_mask in enumerate(mask):
#             #     if not batch_mask:
#             #         vox_with_query[i] = vox_with_query[i]*vox_with_query[i].new_zeros(vox_with_query[i].shape)+ vox_feat[i]
...
#         # return self.lin_self(x) +( gate* inputs )
#         # return inputs + gate * (self.lin_self(x))
...
class CrossAttentionLayer(MessagePassing):
    ...
    def forward(self,
        ...
        if voxel_pos_embedding is not None:
            x_target_with_pos = x_target + voxel_pos_embedding.permute(0,2,3,4,1).flatten(0,3)
