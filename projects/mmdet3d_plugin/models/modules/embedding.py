...
class SingleInputEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed = nn.Sequential(
            nn.Linear(in_channel, middle_channel),
            nn.LayerNorm(middle_channel),
            nn.ReLU(inplace=True),
            nn.Linear(middle_channel, middle_channel),
            nn.LayerNorm(middle_channel),
            nn.ReLU(inplace=True),
...
class MultipleInputConv(nn.Module):

    def __init__(self,
                 in_channels: List[int],
                 out_channel: int) -> None:
        super(MultipleInputConv, self).__init__()
        self.module_list = nn.ModuleList(
            [nn.Sequential(nn.Conv2d(in_channel, out_channel,kernel_size=3,stride=1,padding=1),
...
class SingleInputConv(nn.Module):

    def __init__(self,
                 in_channel: int,
                 out_channel: int) -> None:
        super(SingleInputConv, self).__init__()
        self.embed = nn.Sequential(
