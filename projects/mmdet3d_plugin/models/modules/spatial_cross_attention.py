...
@ATTENTION.register_module()
class MSDeformableAttention3D_DFA3D(MSDA3D):
...
class SpatialCrossAttention_DFA3D(SpatialCrossAttention):
    ...
    Args:
        ...
        deformable_attention: (dict): The config for the deformable attention used in SCA.
    """
    def __init__(self, embed_dims=256, num_cams=6, bev_h=200, bev_w=200, use_empty=False, num_head=8, pc_range=None, dropout=0.1, init_cfg=None, batch_first=False, deformable_attention=dict(type='MSDA3D', embed_dims=256, num_levels=4), **kwargs):
...
class DFA3D(BaseModule):
    ...
    def forward(self,
        ...
        #                                            self.num_points)
        spatial_shapes_3D = self.get_spatial_shape_3D(spatial_shapes, dim_depth)
        cam_mask = ((sampling_locations[..., 1:2] > 0.0)
                    & (sampling_locations[..., 1:2] < 1.0)
                    & (sampling_locations[..., 0:1] < 1.0)
                    & (sampling_locations[..., 0:1] > 0.0))
        
        # sampling_locations [bs, num_query, num_heads,  num_points, num_cams, 3] -> [bs*num_cams, num_query, num_heads, num_levels, num_points, 3]
        b,num_query, num_heads, num_points, num_cams, _ = sampling_locations.shape
        cam_mask = cam_mask.permute(0,4,1,2,3,5).reshape(b*num_cams, num_query, num_heads, num_points).flatten(2).sum(-1) > 0

        sampling_locations = sampling_locations.permute(0,4,1,2,3,5).reshape(b*num_cams, num_query, num_heads, 1, num_points, 3).repeat(1,1,1,self.num_levels,1,1)
        attention_weights = attention_weights.permute(0,3,1,5,4,2).reshape(b*num_cams, num_query, num_heads, self.num_levels, num_points)
        ...
        # sampling_locations [bs, num_query, num_heads, num_points, num_Z_anchors, xy]
        ...
        if not self.batch_first:
            output = output.permute(1, 0, 2)
        output_zero = output.new_zeros(output.shape[0], output.shape[1], output.shape[2])
        output = torch.where(cam_mask.unsqueeze(-1), output, output_zero)
        output = output.reshape(b, num_cams, num_query, -1).sum(dim=1)
