...
class Cross_VoxFormerEncoder3D(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar=4, return_intermediate=False,positional_encoding=None,dataset_type='nuscenes',use_mask_token=False,use_downsample=False,
                 down_layer=None,up_layer=None,
                 **kwargs):

        super(Cross_VoxFormerEncoder3D, self).__init__(*args, **kwargs)
    ...
    @force_fp32(apply_to=())
    def forward(self,src:torch.Tensor,target=None,metas=None):
        ...
        pdb.set_trace()
        if target is None:
            target = self.mask_embed.weight.view(1,1,1,1,C).expand(B,Y,X,Z,C).to(src.dtype)
        
        spatial_shapes = torch.tensor([Y, X, Z], device=src.device).to(dtype=torch.long)
        src = src.view(B,Y*X*Z,C).permute(1,0,2)  # (num_query, bs, embed_dims)
        target = target.view(B,Y*X*Z,C).permute(1,0,2)  # (num_query, bs, embed_dims)
        
        # output = voxel_feat
        voxel_pos = self.positional_encoding(torch.zeros((B,Y*Z,X), device=target.device).to(target.dtype)).to(target.dtype) # [B, dim, 128*4, 128]
        voxel_pos = voxel_pos.flatten(2).permute(2, 0, 1) # [128*4*128, B, dim]
        intermediate = []
        ref_3d = self.get_reference_points(
            Y, X, Z, dim='3dCustom', bs=target.size(1), device=target.device, dtype=target.dtype)
...
class Cross_VoxFormerLayer3D(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
                 ...
                 ffn_num_fcs=2,
                 **kwargs):
        super(Cross_VoxFormerLayer3D, self).__init__(
            attn_cfgs=attn_cfgs,
        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    query,
                    value = torch.cat([prev_bev, query], 0) if prev_bev is not None else query,
