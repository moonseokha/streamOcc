...
# # #             if use_temporal:
# # #                 self.occ_net = nn.ModuleList([nn.Linear(embed_dims,1) for _ in range(temporal_layers + current_layers)])
# # #             else:
...
# # #                         [
# # #                             nn.Linear(embed_dims, num_groups * num_cams * num_levels * num_points) for _ in range(temporal_layers + current_layers)
# # #                         ]
# # #                     )
# # #                     self.sampling_offsets = nn.ModuleList(
# # #                         [
# # #                             nn.Linear(embed_dims,  num_points * 3) for _ in range(temporal_layers + current_layers)
# # #                         ]
# # #                     )
# # #                     self.out_proj = nn.ModuleList([nn.Linear(embed_dims,embed_dims) for _ in range(temporal_layers + current_layers) ])
# # #                     self.proj_drop = nn.ModuleList([nn.Dropout(0.1) for _ in range(temporal_layers + current_layers)])
# # #                 else:
# # #                     self.attention_weights = nn.ModuleList(
# # #                         [
# # #                             nn.Linear(embed_dims, num_groups * num_cams * num_levels * num_points) for _ in range(current_layers)
# # #                         ]
# # #                     )
# # #                     self.sampling_offsets = nn.ModuleList(
# # #                         [
...
#                     self.attention_weights = nn.ModuleList(
#                         [
#                             nn.Linear(embed_dims, num_groups * num_cams * num_levels * num_points) for _ in range(temporal_layers)
#                         ]
#                     )
#                     self.sampling_offsets = nn.ModuleList(
#                         [
...
#                 bs, C, D, H, W = query.shape
#                 if self.use_occ_loss or self.img_to_voxel:
#                     query = query.permute(0,4,3,2,1).flatten(1,3) # (bs, C, D, H, W) -> (bs, W, H, D, C) -> (bs, W*H*D, C)
#                     temp_occ = self.occ_net[i](query)
#                     if self.use_occ_loss:
#                         occ_list.append(temp_occ)
#                     if self.img_to_voxel:
...
#                         occupied_query = occupied_query + features
#                         query = torch.scatter(query,1,occupied_indices.repeat(1,1,query.shape[2]),occupied_query)
#                     query = query.view(bs,W,H,D,C).permute(0,4,3,2,1) # (bs, W*H*D, C) -> (bs, C, D, H, W)
#                     # if self.img_to_voxel:
#                     #     occupancy_scores = temp_occ.sigmoid()
#                     #     occupied_indices = occupancy_scores.topk(int(self.top_k_ratio*occupancy_scores.shape[1]),dim=1)[1]

#                     #     vox_x = (occupied_indices // (self.grid_size[2] * self.grid_size[1]))
#                     #     vox_y = (occupied_indices - vox_x*(self.grid_size[2] * self.grid_size[1])) // self.grid_size[2]
#                     #     vox_z = occupied_indices % self.grid_size[2]
...
#                     #         bs, -1,  self.num_points*self.num_cams*self.num_levels * self.num_groups)
#                     #     attention_weights = attention_weights.softmax(-2).view(bs, -1, self.num_points, self.num_cams, self.num_levels, self.num_groups)
#                     #     offset_normalizer = torch.tensor([self.grid_size[0], self.grid_size[1],self.grid_size[2]]).to(query.device)

#                     #     if vox_pos.dim() == 2:
#                     #         vox_pos = vox_pos.unsqueeze(0)
#                     #     sampling_locations = vox_pos[:, :, None, :] + sampling_offsets / offset_normalizer[None, None, None, :]
#                     #     bev_resolution = torch.tensor([self.bev_resolution[0], self.bev_resolution[1], self.bev_resolution[2]]).to(query.device)
#                     #     bev_origin = torch.tensor([self.bev_origin[0], self.bev_origin[1], self.bev_origin[2]]).to(query.device)
#                     #     key_points = sampling_locations * bev_resolution[None, None, None, :] + bev_origin[None, None, None, :]
#                     #     points_2d = (
#                     #         self.project_points(
#                     #             key_points, #[bs, num_anchor, num_pts, 3]
...
#                     #     occupied_query = occupied_query + features
#                     #     query = torch.scatter(query,1,occupied_indices.repeat(1,1,query.shape[2]),occupied_query)
#                     # query = query.view(bs,H,W,D,C).permute(0,4,3,2,1) # (bs, W*H*D, C) -> (bs, C, D, H, W)
#             if self.transformer is not None:
#                 bs,_,occ_z, occ_w, occ_h = query.shape
#                 dtype = query.dtype
#                 occ_feature = query.permute(0,1,3,4,2)
...
class Vox_Convnet(nn.Module):
    ...
    def forward(self,
        ...
        occ_mask = torch.zeros((B, H, W, D),
                                device=voxel_feat.device).to(dtype)
        # if self.transformer is not None:
        ...
        if self.use_temporal:
            ...
            if self.img_to_voxel:
                ...
                zs = torch.linspace(0.5, D - 0.5, D, dtype=dtype,
                    device=device).view(D, 1, 1).expand(D, H, W) / D
                xs = torch.linspace(0.5, W - 0.5, W, dtype=dtype,
                                    device=device).view(1, 1, W).expand(D, H, W) / W
                ys = torch.linspace(0.5, H - 0.5, H, dtype=dtype,
                                    device=device).view(1, H, 1).expand(D, H, W) / H
                ref_3d = torch.stack((xs, ys, zs), -1) # (W,H,Z,3)
                ref_3d = ref_3d.permute(3, 0, 1, 2).flatten(1).permute(1, 0)
                ref_3d = ref_3d[None, None].repeat(B, 1, 1, 1)
            ...
            for i in range(self.temporal_layers):
                ...
                if self.use_occ_loss or self.img_to_voxel:
                    ...
                    if self.img_to_voxel:
                        ...
                        vox_pos_embedding = []
                        for b in range(bs):
                            vox_indices.append(torch.index_select(ref_3d.squeeze(1)[b],0,occupied_indices[b].squeeze(1)).unsqueeze(0))  
