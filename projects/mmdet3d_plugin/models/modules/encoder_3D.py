...
class VoxFormerEncoder3D(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar=4, return_intermediate=False,positional_encoding=None,dataset_type='nuscenes',use_mask_token=False,use_downsample=False,
                 down_layer=None,up_layer=None,use_temporal=False,
                 **kwargs):
    ...
    def forward(self,
        ...
        if self.use_temporal:
            if prev_bev is not None:
                prev_bev = prev_bev.reshape(bs,len_bev,-1)
            else:
                prev_bev = torch.zeros_like(voxel_feat)
            ref_3d = ref_3d.repeat(2, 1, 1, 1)
            prev_bev = self.projection(prev_bev)
        else:
            prev_bev = None
        ...
        for lid, layer in enumerate(self.layers):
            output,occ = layer(
                ...
                prev_bev=prev_bev,
                metas=metas,
                prev_metas=prev_metas if self.use_temporal else None,
                img_feats=img_feats)
    ...
    def forward(self,
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                if prev_bev is not None:
                    prev_curr_bev = torch.cat([prev_bev, query], dim=0)
                query = self.attentions[attn_index](
                    query,
                    value = query if prev_bev is None else prev_curr_bev,
